## Descriptive functions designed by Scott Emerson
...
descrip <- function (..., strata=NULL, subset=NULL, probs= c(.25,.50,.75), geomInclude=FALSE, replaceZeroes=FALSE,
  ...
  #     - if restriction==Inf, the largest observation time will be used as the restriction
  # above, below, labove, rbelow, lbetween, rbetween define thresholds for cdf or survivor functions
  #     - for above, below, labove, lbelow, -Inf and Inf are appended, and the sorted unique values are then used as cutpoints
  #     - the proportions are defined for
  #             x > above[j]
  #             x >= labove[j]
  #             x < below[j]
  #             x <= rbelow[j]
  ...
  #      Msng             the number of missing values for observations
  #      Mean             the (restricted) mean
  #      Std Dev          the (restricted) standard deviation
  #      Gm Mean          the (restricted) geometric mean
  #      Min              the (possibly censored) minimum
  #      quantiles        quantiles corresponding to the values in probs
  #      Max              the (possibly censored) maximum
  #      probabilities    the probabilities (possibly derived from KM) for intervals defined by above, labove,
  #                            below, rbelow, lbetween, rbetween, interval, linterval, rinterval, lrinterval
  ...
  vDescrip <- function (x, probs, thresholds, geometricMean, geomInclude, replaceZeroes) {
    ...
    if (rslt==0) {
      ...
      if (length(x)==0 | is.character(x)) {
        ...
        if (ntholds>0) {
          for (j in 1:ntholds) {
            u <- ifelse1(thresholds[j,1]==0, x > thresholds[j,2], x >= thresholds[j,2]) &
              ifelse1(thresholds[j,3]==0, x < thresholds[j,4], x <= thresholds[j,4])
            rslt <- c(rslt,mean(u))
          }
        }
      ...
      tnames <- paste(sep="","Pr",
                      ifelse(thresholds[,2] == -Inf,
                             ...
                             ifelse(thresholds[,4]==Inf,
                                    ...
                                    paste(sep="",ifelse(thresholds[,1]==0,"(","["),format(tholds[,2]),",",format(tholds[,4]),
                                          ifelse(thresholds[,3]==0,")","]")))))
    }
    # includes geometric mean if asked
    if(geomInclude){
      dimnames (rslt) <- list("",c("N", "Msng", "Mean", "Std Dev", "Geom Mn",
                                   qnames, tnames, "restriction", "firstEvent", "lastEvent", "isDate"))
    } else { #otherwise doesn't
      dimnames (rslt) <- list("",c("N", "Msng", "Mean", "Std Dev",
                                   qnames, tnames, "restriction", "firstEvent", "lastEvent", "isDate"))
    }
    # if(sum(is.na(rslt)) > 0){
    #   print("Contains missing values, descriptive statistics for some attributes are calculated by deleting the missing occurances")
    # }
    rslt
  }
  ...
  sDescrip <- function (x, probs, thresholds, geomInclude, geometricMean, replaceZeroes, restriction) {
    ...
    if (rslt==0) {
      ...
      if (dim(x)[1]==0) {
        ...
        if (geometricMean) {
          x2 <- x
          x2[,1] <- ifelse(x2[,1]==0,log(replaceZeroes),log(x2[,1]))
          z2 <- KM(x2)
        ...
        if (ntholds>0) {
          for (j in 1:ntholds) {
            rslt <- c(rslt,
                      ifelse1(thresholds[j,1]==0, sKM(z,thresholds[j,2]), sKM(z,thresholds[j,2],F)) -
                        ifelse1(thresholds[j,4]==Inf, 0,
                                ifelse1(thresholds[j,3]==0, sKM(z,thresholds[j,4],F), sKM(z,thresholds[j,4]))))
          }
        }
        rslt <- c(rslt,attr(tmp1,"restriction")[2],firstEvent,lastEvent)
      }
    }
    rslt <- matrix(c(rslt,0),1)
    qnames <- paste(format(100*probs),"%",sep="")
  ...
  vStrdescr <- function (x, stratav, subsetv, probs, thresholds, geomInclude, replaceZeroes) {
    ...
    if (is.null(stratav)) stratav <- rep(1,length(x))
  ...
  dStrdescr <- function (x, stratad, subsetd, probs, threshholds, geomInclude, replaceZeroes) {
    ...
    xi <- as.integer(x)
    rslt <- vStrdescr (xi, stratad, subsetd, probs, thresholds, geomInclude, replaceZeroes)
    rslt[,"isDate"] <- 1
    rslt
  }
  ...
  sStrdescr <- function (x, stratas, subsets, probs, thresholds, geomInclude, replaceZeroes, restriction) {
    ...
    if (is.null(stratas)) stratas <- rep(1,n)
    if (n != length(stratas)) stop("length of variables must match length of strata")
    x <- x[subsets]
    if(geomInclude){
    ...
    if (is.logical(replaceZeroes)) {
      if (replaceZeroes) replaceZeroes <- min(x[!is.na(x) & x[,1] > 0,1]) / 2 else replaceZeroes <- NA
    }
  ...
  mStrdescr <- function (x, stratam, subsetm, probs, thresholds, geomInclude, replaceZeroes) {
    ...
    #rslt <- apply(x, 2, FUN=function(x2) vStrdescr(x2, stratam, subsetm, probs, thresholds, replaceZeroes))
    rslt <- NULL
    for (i in 1:p) {
      rslt <- rbind(rslt, vStrdescr(x[,i], stratam, subsetm, probs, thresholds, geomInclude, replaceZeroes))
    }
  ...
  if (length(lbetween)>0) {
    ...
    thresholds <- rbind(thresholds,cbind(1,lbetween[-length(lbetween)],0,lbetween[-1]))
  }
  if (length(rbetween)>0) {
    rbetween <- sort(unique(c(-Inf,rbetween,Inf)))
    thresholds <- rbind(thresholds,cbind(0,rbetween[-length(rbetween)],1,rbetween[-1]))
  }
  if (!is.null(interval)) {
  ...
  for (i in 1:p) {
    ...
    if (is.list(x)) {
      ...
      nV <- nV + 1
      rslt <- rbind(rslt, dStrdescr(x, strata, subset, probs, thresholds, geomInclude, replaceZeroes))
    } else if (is.factor(x)) {
      x <- list(x)
      nV <- nV + 1
      names(x) <- nms[nV]
      rslt <- rbind(rslt, lStrdescr(x, strata, subset, probs, thresholds, geomInclude, replaceZeroes))
    } else {
      x <- matrix(x)
      nV <- nV + 1
