correlate <-
function (..., strata=NULL, subset=NULL, conf.level= 0.95, use="pairwise.complete.obs", 
                       method="pearson", stat="cor", byStratum=TRUE, version=FALSE) {
  ...
  corr <- function(z, na.rm=T, conf.level= 0.95, method="pearson") {
    if (!is.matrix(z)) stop ("corr must take a matrix z")
    p <- dim(z)[2]
    nms <- dimnames(z)[[2]]
    if(method=="spearman") {
    ...
    cov <- matrix((one %*% w - ((one %*% z1) * (one %*% z2))/n)/sqrt(vx * vy), nrow = p)
    tst <- cov/sqrt((1 - cov^2)/(nn - 2))
    tst[cov == 1 | cov == -1] <- 0
    z <- log ((1+cov)/(1-cov)) / 2
    lo <- hi <- p <- rep(NA, length(n))
    if (any(!is.na(nn))) {
      for(m in unique(n[!is.na(nn)]))
        p[n == m] <- 2 * pt( - abs(tst[n == m]), m - 2)
    }
    if (any(n > 3)) {
      for(m in unique(n[n > 3])) {
        lo[n == m] <- z[n == m] - qnorm(conf.level/2 + 0.5) / sqrt(m-3)
        hi[n == m] <- z[n == m] + qnorm(conf.level/2 + 0.5) / sqrt(m-3)
      }
    }
    ...
    lo[n ==0] <- NaN
    hi[n ==0] <- NaN
    dimnames(cov) <- dimnames(n) <- dimnames(tst) <- dimnames(p) <- dimnames(lo) <- dimnames(hi) <-
      list(nms, nms)
    rslt <- list(cormtx = cov, n = n, t.stat = tst, pval = p, lo=lo,hi=hi)
    names(rslt)[5:6] <- paste(c("lo","hi"),format(100*conf.level),"%CI",sep="")
    rslt
  }
  
  chc.use <- c("everything","complete.obs","pairwise.complete.obs")
  use <- pmatch(use, chc.use)
  if (is.na(use)) stop("unrecognized value for use")
  chc.method <- c("pearson","spearman")
  method <- pmatch(method,chc.method)
  if (is.na(method)) stop("unrecognized value for method")
  ...
  if (is.null(X)) stop("no variables for correlations")
  if (dim(X)[1]==1) stop("must have more than 1 variable for correlations")
  if (use == 2) {
    deleteCase <- apply(is.na(X),1,any)
  } else deleteCase <- rep(F,dim(X)[1])
  if (!is.null(subset)) {
    if (is.logical(subset)) keepCase <- (1:(dim(X)[1]))[subset & !deleteCase]
    else if (is.integer(subset)) {
      subset <- subset[!is.na(subset)]
      if (length(subset)==0) stop("improper format for subset")
      if (all(subset > 0)) {
        keepCase <- subset[!(subset %in% (1:(dim(X)[1]))[deleteCase])]
      } else if (all(subset < 0)) {
        keepCase <- (1:(dim(X)[1]))[unique(c(subset,-(1:(dim(X)[1]))[deleteCase]))]
  ...
  if (!is.null(strata)) {
    for (s in sort(unique(strata))) {
      indx <- strata==s
      rslt <- c(rslt,list(corr(X[indx,],na.rm=na.rm, conf.level=conf.level, method=method)))
    }
    names(rslt) <- sort(unique(strata))
  }
  rslt <- c(rslt,list(All=corr(X,na.rm=na.rm, conf.level=conf.level, method=method)))
